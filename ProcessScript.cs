    ...
    public partial class P_Pokazateli1S_Scripts : EleWise.ELMA.Workflow.Scripts.ProcessScriptBase<Context>
    {
        ...
        public void UpdateMetricsData(Context context)
        {
            ...
            foreach (var lab in labs)
            {
                ...
                context.Log += lab.Naimenovanie + Environment.NewLine;
        ...
        public void UpdateShareSotrudnikBefore39(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            foreach (var data in skResearcher)
            {
                ...
                if (data.Sotrudnik.Ssylka1sRD.FizicheskoeLico.DataRozhdeniya == null)
                    continue;
                var dataBirthday = data.Sotrudnik.Ssylka1sRD.FizicheskoeLico.DataRozhdeniya; //var dataBirthday = data.Sotrudnik.Ssylka1sRD.FizicheskoeLico.DataRozhdeniya;
        ...
        public void UpdateShareSotrudnikCandidateCount(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var workSotrudniks = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).Where(x => x.DataUvoljneniya == null);
            var candidate = workSotrudniks.Where(x => !string.IsNullOrEmpty(x.Sotrudnik.UchenayaStepenj) && x.Sotrudnik.UchenayaStepenj.ToLower().Contains("кандидат"));
        ...
        public void UpdateShareSotrudnikNPR(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var sotrudnuksKafedra = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find("Podrazdelenie in (Tip LIKE '%кафедр%')").Where(x => x.DataUvoljneniya == null).Select(x => x.KodSotrudnika).GroupBy(x => x);

            var workSotrudnikslab = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).Where(x => x.DataUvoljneniya == null).Select(x => x.KodSotrudnika).GroupBy(x => x);

            var sotrudnicsNPR = workSotrudnikslab.Select(x => x.Key).Intersect(sotrudnuksKafedra.Select(x => x.Key));
            context.Log += sotrudnuksKafedra.Count() + Environment.NewLine;
            context.Log += workSotrudnikslab.Count() + Environment.NewLine;
            context.Log += sotrudnicsNPR.Count() + Environment.NewLine;
            ...
            double val = (((double)sotrudnicsNPR.Count() / (double)workSotrudnikslab.Count()) * 100);
            if (Double.IsNaN(val))
            {
                val = 0;
            }
            ...
            //							continue;
            //							
            //						if(itemSk.Podrazdelenie.Tip.Value.ToString().ToLower().Contains("кафедр"))
            //						{
            //							countUserNpf++;
            //							break;
            //						}
        ...
        public void UpdateRID(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var workSk = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).GroupBy(x => x.Sotrudnik);
            var rids1 = PublicAPI.Objects.UserObjects.UserRezuljtatIntellektualjnoyDeyateljnostiRID.Find("VidRID = 'Патент на изобретение' OR VidRID = 'Патент на полезную модель'").ToList().OrderBy(x => x.NazvanieRID);
            string result1 = "";
            foreach (var element in rids1)
            {
                if (workSk.Any(x => element.Avtory.Contains(x.Key)))
                    result1 += element.NazvanieRID + ";" + Environment.NewLine;
            }
            patent.Znachenie = result1;
            context.Pokazateli.Last().PokazateliLaboratorii.Add(patent);
            //patent.Znachenie = "";
            ...
            var rids2 = PublicAPI.Objects.UserObjects.UserRezuljtatIntellektualjnoyDeyateljnostiRID.Find("VidRID = 'Свидетельство о гос.регистрации программы для ЭВМ' OR VidRID = 'Свидетельство о гос.регистрации базы данных'").ToList().OrderBy(x => x.NazvanieRID);
            string result2 = "";
            foreach (var element in rids2)
            {
                if (workSk.Any(x => element.Avtory.Contains(x.Key)))
                    result2 += element.NazvanieRID + ";" + Environment.NewLine;
            }
            metric.Znachenie = result2;
        ...
        public void UpdateExecuteRID(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var sotrudniks = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).GroupBy(x => x.Sotrudnik);
            var vNioktr = PublicAPI.Objects.UserObjects.UserUchastieVNIOKTR.Find(string.Format("Laboratoriya = {0}", lab.Id)).Select(x => x.NIOKTR).Where(x => x.VidNIOKTR.Naimenovanie == "Хоз. договор" || x.VidNIOKTR.Naimenovanie == "Госзадание" && x.CreationDate.Year == DateTime.Now.Year).GroupBy(x => x.ObjemPostupivshihSredstvOtVypolneniyaNIOKTRRubVGod);
            //context.Log += vRid;
            foreach (var element in vNioktr)
            {
                context.Log += "Объем Ниоктр за год";
                context.Log += element.Key + Environment.NewLine;
            }
            ...
            //var metric = GetOrCreateMetrick (lab, METRIC_SCIENCE_USE_RID);
            var workSk = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).GroupBy(x => x.Sotrudnik);
            var nioktr = PublicAPI.Objects.UserObjects.UserUchastieVNIOKTR.Find(string.Format("Laboratoriya = {0}", lab.Id)).Select(x => x.NIOKTR).Where(x => x.VidNIOKTR.Naimenovanie == "Хоз. договор" || x.VidNIOKTR.Naimenovanie == "Госзадание").ToList();
            var sumNIOKTR = Convert.ToDouble(nioktr.Select(x => x.ObjemPostupivshihSredstvOtVypolneniyaNIOKTRRubVGod).Sum());
            var rid = PublicAPI.Objects.UserObjects.UserRezuljtatIntellektualjnoyDeyateljnostiRID.Find(FetchOptions.All).ToList().OrderBy(x => x.NazvanieRID);
            var result = 0.0;
            foreach (var element in rid)
            {
                if (!workSk.Any(x => element.Avtory.Contains(x.Key)))
                    result += Convert.ToDouble(element.ObjemSredstvPoluchennyhUniversitetomPriIspoljzovaniiRIDVGod);
            }
            double val = ((result / (sumNIOKTR + result)) * 100);
            if (Double.IsNaN(val))
            {
                val = 0;
            }
        ...
        public void UpdatePublications(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var workSk = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).GroupBy(x => x.Sotrudnik);
            var publish = PublicAPI.Objects.UserObjects.UserPublikacii.Find(FetchOptions.All).ToList();
            string result = "";
            foreach (var element in publish)
            {
                var avtory = element.Avtory.ToList().Select(x => x.Sotrudnik);
                if (workSk.Any(x => avtory.Contains(x.Key)))
                    result += element.Nazvanie + ";" + Environment.NewLine;
            }
            metric.Znachenie = result;
            //metric.Znachenie = "";
        ...
        public void UpdateListDokladRF(Context context, EleWise.ELMA.ConfigurationModel.Podrazdeleniya lab)
        {
            ...
            var workSk = PublicAPI.Objects.UserObjects.UserKadrovayaIstoriyaSotrudnika.Find(string.Format("Podrazdelenie = {0}", lab.Id)).GroupBy(x => x.Sotrudnik);
            var doclads = PublicAPI.Objects.UserObjects.UserUchastieVKonferenciyah.Find(FetchOptions.All).ToList().OrderBy(x => x.DataDoklada);
            string result1 = "";
            string result2 = "";
            foreach (var element in doclads)
            {
                if (workSk.Any(x => element.Dokladchik == x.Key && element.Konferenciya.CtatusKonferencii.Value.Contains("Международная")))
                {
                    result1 += element.NazvanieDoklada + ";" + Environment.NewLine;
                    foreach (var pub in element.Publikaciya)
                    {
                        result2 += pub.Nazvanie + ";" + Environment.NewLine;
                    }
                }
            }
            metric.Znachenie = result1;
            //metric.Znachenie = "";
            context.Pokazateli.Last().PokazateliLaboratorii.Add(metric);
            UpdateHistoryMetric(lab, metric.Pokazatelj, metric.Znachenie);

            var material = GetOrCreateMetrick(lab, METRIC_SCIENCE_EVENT_LIST_MATERIAL);
            material.Znachenie = result2;
            context.Pokazateli.Last().PokazateliLaboratorii.Add(material);
            UpdateHistoryMetric(lab, material.Pokazatelj, material.Znachenie);
        }
